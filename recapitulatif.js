document.addEventListener('DOMContentLoaded', () => {
    ...
    function chargerToutesLesPoules() {
        ...
        for (let n = 1; n <= config.niveaux; n++) {
            const raw = localStorage.getItem(`${config.categorie}-${config.genre}-${n}`);
            if (raw) {
                try {
    ...
    function calculerStatsGlobales(niveauxData) {
        ...
        const totalEquipes  = niveauxData.reduce((s, n) =>
            s + n.poules.reduce((ss, p) => ss + p.equipes.length, 0), 0);
        const distMoyGlobal = niveauxData.reduce((s, n) => {
            const moy = n.poules.reduce((ss, p) => ss + parseFloat(p.distance_moyenne || 0), 0) / (n.poules.length || 1);
            return s + moy;
        }, 0) / (totalNiveaux || 1);
    ...
    function exporterExcel(niveauxData) {
        ...
        const recapRows = [
            ...
            [],
            ['Niveau', 'Nb poules', 'Nb équipes', 'Dist. moy. (km)', 'Dist. min (km)', 'Dist. max (km)', 'Écart (km)'],
        ];
    ...
    if (niveauxManquants.length) {
        toast(`Niveaux sans poules : ${niveauxManquants.map(n => `Niveau ${n}`).join(', ')}`, 'warn');
    }
});
